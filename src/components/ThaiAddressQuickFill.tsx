...
import { searchThaiAddresses, toQuickFillValue } from '../lib/search.js';
...
export function ThaiAddressQuickFill({
  ...
  const results = useMemo(() => {
    ...
    return searchThaiAddresses({ query, limit }, 'full');
  }, [query, limit]);
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'grid', gap: 12, gridTemplateColumns: 'repeat(2, minmax(0, 1fr))' }}>
        <Field label={labels?.subdistrict ?? 'Subdistrict'} value={selected?.subdistrict ?? ''} accent />
        <Field label={labels?.district ?? 'District'} value={selected?.district ?? ''} />
        <Field label={labels?.province ?? 'Province'} value={selected?.province ?? ''} />
        <Field label={labels?.postalCode ?? 'Postal code'} value={selected?.postalCode ?? ''} mono />
      </div>
    </div>
  );
}
